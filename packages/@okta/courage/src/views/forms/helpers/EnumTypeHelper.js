...
define([
  ...
  function enumObjectToValue(obj) {
    ...
    return convertIndexToEnumIndex(_.findIndex(this.options.params.enumOneOf, function (oneOfObj) {
  ...
  function valuesToEnumObjects(values) {
    return _.chain(values)
      .map(valueToEnumObject, this)
      .filter(function (val) { return !_.isNull(val); })
      .value();
  }

  function enumObjectsToValues(values) {
    return _.chain(values)
      .map(enumObjectToValue, this)
