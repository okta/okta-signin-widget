...
console.debug('load playground main');
...
declare global {
  interface Window {
    ...
    createWidgetInstance: (options: any) => any;
    renderPlaygroundWidget: (options: any) => void;
    additionalOptions?: Partial<any>;
  }
}

function isSuccessNonOIDC(res) {
  return (res).session !== undefined;
}
...
function createWidgetInstance(options: any = {}) {
  console.debug('creating siw instance 005');
  if (siw) {
    siw.remove();
  }
