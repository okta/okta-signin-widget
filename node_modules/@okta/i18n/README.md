...
> This flow will be updating soon when the [last piece](https://oktainc.atlassian.net/browse/OKTA-81960) of the promotion flow is ironed out.
...
2. Find the beta prerelease version of the artifact you just published

    ```bash
    # Only run this when the master branch has finished the publish test suite
    # The last beta version will be the one you want, i.e. 1.3.0-beta.3
    $ npm view @okta/i18n versions
    ```

3. Update version in all affected downstream modules. In okta-core, use the [install-with-shrinkwrap](https://github.com/okta/npm/tree/master/install-with-shrinkwrap#install-with-shrinkwrap) package to generate the right *npm-shrinkwrap.json* file in both *clients/admin* and the root *okta-core* folder:

    ```bash
    # If you're updating okta-core, use the installsw command in clients/admin and
    # the root okta-core folder. For example, if your beta version is 1.3.0-beta.3:
    [okta-core/clients/admin]$ npm run installsw @okta/i18n@1.3.0-beta.3
    [okta-core]$ npm run installsw @okta/i18n@1.3.0-beta.3

    # If you're updating a module that is not using install-with-shrinkwrap:
    [your-module]$ npm install -E @okta/i18n@<your-beta-version> --save

    # i.e. if your version is 1.3.0-beta.3:
    [okta-core/clients/admin]$ npm install -E @okta/i18n@1.3.0-beta.3 --save
    ```
