...
test.requestHooks(identifyRequestLogger, identifyMock)('should always trim identifier', async t => {
  const identityPage = await setup(t);
  await checkA11y(t);

  await identityPage.fillIdentifierField('   Test Identifier@okta.com ');
  await identityPage.clickNextButton();
...
test.requestHooks(identifyRequestLogger, identifyMock)('should trim identifier first and transform identifier using settings.transformUsername', async t => {
  const identityPage = await setup(t, {
