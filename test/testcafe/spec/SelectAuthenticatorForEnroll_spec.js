...
test.requestHooks(mockEnrollAuthenticatorPassword)('should load select authenticator list', async t => {
  ...
  await t.expect(selectFactorPage.getFactorLabelByIndex(9)).eql('Atko Custom OTP Authenticator');
  await t.expect(selectFactorPage.getFactorIconClassByIndex(9)).contains('mfa-hotp');
  await t.expect(selectFactorPage.getFactorSelectButtonByIndex(9)).eql('Set up');
  await t.expect(selectFactorPage.getFactorSelectButtonDataSeByIndex(9)).eql('custom_otp');
  await t.expect(selectFactorPage.getFactorDescriptionByIndex(9)).eql('Enter a temporary code generated from an authenticator device.');
