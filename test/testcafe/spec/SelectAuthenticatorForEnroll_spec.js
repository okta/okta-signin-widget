...
test.requestHooks(mockEnrollAuthenticatorPassword)('should load select authenticator list', async t => {
  ...
  await t.expect(selectFactorPage.getFactorSelectButtonDataSeByIndex(6)).eql('del_oath-otp');
  ...
  await t.expect(selectFactorPage.getFactorLabelByIndex(7)).eql('Duo Security');
  await t.expect(selectFactorPage.getFactorIconClassByIndex(7)).contains('mfa-duo');
  await t.expect(selectFactorPage.getFactorSelectButtonByIndex(7)).eql('Set up');
  await t.expect(selectFactorPage.getFactorSelectButtonDataSeByIndex(7)).eql('duo_native');
  await t.expect(selectFactorPage.getFactorDescriptionByIndex(7)).eql('Verify your identity using Duo Security.');
