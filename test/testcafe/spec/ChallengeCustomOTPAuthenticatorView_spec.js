...
test.requestHooks(mockChallengeAuthenticatorCustomOTP)('challenge custom OTP authenticator', async t => {
  ...
  // verify otp
  await challengeCustomOTPPage.verifyFactor('credentials.otp', '1234');
...
test.requestHooks(mockChallengeAuthenticatorCustomOTP)('OTP is required', async t => {
  ...
  // verify otp
  await challengeOnPremPage.verifyFactor('credentials.otp', '');
...
test.requestHooks(mockInvalidPasscode)('challege on prem authenticator with invalid passcode', async t => {
  const challengeOnPremPage = await setup(t);
  await challengeOnPremPage.verifyFactor('credentials.otp', 'test');
  await challengeOnPremPage.clickNextButton();
