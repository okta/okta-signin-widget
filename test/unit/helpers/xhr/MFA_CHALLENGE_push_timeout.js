export default {
  status: 200,
  responseType: 'json',
  response: {
    stateToken: '00uvJlu1Kvehw-cwXY3CTH5gxisDXQ4p58XgC1xW0s',
    expiresAt: '2015-10-22T02:23:28.893Z',
    status: 'MFA_CHALLENGE',
    factorResult: 'TIMEOUT',
    _embedded: {
      user: {
        id: '00upmqsv15YoFBHHw0g3',
        passwordChanged: '2015-10-18T22:47:19.000Z',
        profile: {
          login: 'test@asdf.com',
          firstName: 'test',
          lastName: 'test',
          locale: 'en_US',
          timeZone: 'America/Los_Angeles',
        },
      },
      factor: {
        id: 'opfsx1KrcmkmCu71b0g3',
        factorType: 'push',
        provider: 'OKTA',
        profile: {
          credentialId: 'test@asdf.com',
          deviceType: 'SmartPhone_IPhone',
          keys: [
            {
              kty: 'PKIX',
              use: 'sig',
              kid: 'default',
              x5c: [
                '',
              ],
            },
          ],
          name: 'Tom iphone',
          platform: 'IOS',
          version: '9.0.2',
        },
        _embedded: {
          verification: null,
        },
      },
    },
    _links: {
      next: {
        name: 'verify',
        href: 'https://ntate.okta1.com:80/api/v1/authn/factors/opfsx1KrcmkmCu71b0g3/verify',
        hints: {
          allow: ['POST'],
        },
      },
      cancel: {
        href: 'https://ntate.okta1.com:80/api/v1/authn/cancel',
        hints: {
          allow: ['POST'],
        },
      },
      prev: {
        href: 'https://ntate.okta1.com:80/api/v1/authn/previous',
        hints: {
          allow: ['POST'],
        },
      },
      resend: [
        {
          name: 'push',
          href: 'https://ntate.okta1.com:80/api/v1/authn/factors/opfsx1KrcmkmCu71b0g3/verify/resend',
          hints: {
            allow: ['POST'],
          },
        },
      ],
    },
  },
};
