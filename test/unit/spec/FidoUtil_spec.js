...
function (Okta, Expect, FidoUtil) {
  Expect.describe('FidoUtil', function () {
    ...
    it('Returns u2f enroll error message', function () {
      expect(FidoUtil.getU2fEnrollErrorMessageByCode(1))
        .toEqual('An unknown error has occured. Try again or select another factor.');
    });

    it('Returns u2f verify error message', function () {
      expect(FidoUtil.getU2fVerifyErrorMessageByCode(1, true))
        .toEqual('An unknown error has occured. Try again or contact your admin for assistance.');
      expect(FidoUtil.getU2fVerifyErrorMessageByCode(1, false))
        .toEqual('An unknown error has occured. Try again or select another factor.');
    });
  });
});
