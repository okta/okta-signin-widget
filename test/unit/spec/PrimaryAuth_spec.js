...
function (_, $, Q, OktaAuth, LoginUtil, Okta, Util, AuthContainer, PrimaryAuthForm, Beacon, PrimaryAuth,
          ...
          resNonJson, resInvalidText, resThrottle, resIdentifierFirst, $sandbox) {
  ...
  Expect.describe('PrimaryAuth', function () {
    ...
    Expect.describe('Identifier First', function() {
      itp('does not have a password field', function () {
        return setup({ 'features.identifierFirst': true }).then(function (test) {
          var password = test.form.passwordField();
          expect(password.length).toBe(0);
        });
      });
      itp('shows an error if identifier field is not an email', function () {
        return setup({ 'features.identifierFirst':  true }).then(function (test) {
          test.form.setUsername('testuser');
          test.form.submit();
          return Expect.waitForFormError(test.form, test);
      ...
      itp('calls authClient primaryAuth with username only', function () {
      ...
      itp('shows MfaVerify view after primary auth', function () {
        return setupIdentifierFirst().then(function (test) {
          test.form.setUsername('testuser@test.com');
          test.form.submit();
          return Expect.waitForMfaVerify(test);
