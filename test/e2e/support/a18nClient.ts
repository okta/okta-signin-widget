...
export default class A18nClient {
  ...
  async getEmailMagicLink(profileId: string) {
    ...
    const match = response?.content?.match(/<a id="(?:email-authentication-button|email-activation-button|registration-activation-link|reset-password-link)" href="(?<url>\S+)"/);
