...
scenario('webauthn-verify-mfa', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/identify', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/identify/webauthn-verify-mfa.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/webauthn-verify-mfa.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
