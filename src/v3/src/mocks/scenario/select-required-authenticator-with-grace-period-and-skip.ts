...
scenario('select-authenticator-with-grace-period-and-skip', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/introspect', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/introspect/authenticator-enroll-grace-period-with-skip.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
