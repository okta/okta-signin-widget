...
scenario('webauthn-enroll-mfa', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/identify', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/identify/webauthn-enroll-mfa.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  rest.post('*/idp/idx/credential/enroll', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/credential/enroll/webauthn-enroll-mfa.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/webauthn-enroll-mfa.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
