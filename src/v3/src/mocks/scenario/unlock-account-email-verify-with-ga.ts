...
scenario('unlock-account-email-verify-with-ga', (rest) => ([
  // bootstrap
  ...
  // answer (sent the email code)
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/unlock-account-email-additional-verification.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  }),
  // enters Google Authenticator MFA flow
  rest.post('*/idp/idx/challenge', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/unlock-account-google-auth-verify.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  }),
  // enters Google Authenticator verification code
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/unlock-account-google-auth-success.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
