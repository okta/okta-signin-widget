...
scenario('unlock-account-sms-verify-with-ga', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/challenge', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/unlock-account-sms-with-ga-verify.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/unlock-account-sms-ga-verify.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  }),
  // answer (sent the Google Authenticator code)
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/unlock-account-sms-ga-verify-success.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
