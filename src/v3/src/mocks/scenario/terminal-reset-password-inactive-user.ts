...
scenario('terminal-reset-password-inactive-user', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/introspect', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/introspect/terminal-reset-password-inactive-user.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
