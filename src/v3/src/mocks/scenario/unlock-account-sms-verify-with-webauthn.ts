...
scenario('unlock-account-sms-verify-with-webauthn', (rest) => ([
  // bootstrap
  ...
  // challenge (sent the sms verification type)
  rest.post('*/idp/idx/challenge', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/unlock-account-sms.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  // answer (sent the SMS code)
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/unlock-account-sms-verify-webauthn.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  }),
  // challenge (sent the WebAuthN verification type)
  rest.post('*/idp/idx/challenge', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/unlock-account-sms-verify-webauthn.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  }),
  // Sends WebAuthN attestation
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/unlock-account-sms-verify-webauthn-success.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
