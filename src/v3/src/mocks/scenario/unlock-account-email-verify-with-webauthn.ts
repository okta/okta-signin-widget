...
scenario('unlock-account-email-verify-with-webauthn', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/unlock-account-email-verify-webauthn.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  }),
  // enters WebAuthN MFA flow
  rest.post('*/idp/idx/challenge', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/unlock-account-email-verify-webauthn.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/unlock-account-email-verify-webauthn-success.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
