...
scenario('authenticator-expiry-warning-password', (rest) => ([
  // bootstrap
  ...
  // expired password with complexity but warning
  rest.post('*/idp/idx/identify', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/identify/authenticator-expiry-warning-password.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
