...
scenario('error-request-not-completed', (rest) => ([
  // bootstrap
  ...
  // submit name/email and returns error
  rest.post('*/idp/idx/enroll/new', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/enroll/new/error-request-not-completed.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
