...
scenario('reset-password-email-securityquestion', (rest) => ([
  // bootstrap
  ...
  // answer (sent the answer) - show password reset form
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/password-reset.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
