...
scenario('google-auth-scan-enroll', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/identify', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/identify/google-auth-scan-enroll.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  rest.post('*/idp/idx/credential/enroll', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/credential/enroll/google-auth-scan-enroll.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/google-auth-scan-enroll.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
