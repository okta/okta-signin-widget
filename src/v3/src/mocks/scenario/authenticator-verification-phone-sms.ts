...
scenario('authenticator-verification-phone-sms', (rest) => ([
  // bootstrap
  ...
  // send mfa type, return method selection
  rest.post('*/idp/idx/challenge', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/sms-method.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
  }),
  // send sms method, return ask for mfa code
  rest.post('*/idp/idx/challenge', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/sms-challenge.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
