...
scenario('terminal-return-otp-only-full-location', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/introspect', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/introspect/terminal-return-otp-only-full-location.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
