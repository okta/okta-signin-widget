...
scenario('terminal-invalid-forgot-password-token', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/introspect', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/introspect/terminal-invalid-forgot-password-token.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
