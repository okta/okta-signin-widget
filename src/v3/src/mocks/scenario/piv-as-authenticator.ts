...
scenario('piv-as-authenticator', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/introspect', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/authenticator-select-piv.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
  ...
  rest.post('*/idp/idx/challenge', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/authenticator-piv-verify.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
