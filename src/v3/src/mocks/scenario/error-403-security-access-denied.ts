...
scenario('error-403-security-access-denied', (rest) => ([
  // bootstrap
  ...
  // display error on load of the widget
  rest.post('*/idp/idx/introspect', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/introspect/error-403-security-access-denied.json');
    return res(
      ctx.status(403),
      ctx.json(body),
    );
