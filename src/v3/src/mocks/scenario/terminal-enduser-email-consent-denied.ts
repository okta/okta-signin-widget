...
scenario('terminal-enduser-email-consent-denied', (rest) => ([
  // bootstrap
  ...
  rest.post('*/idp/idx/introspect', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/introspect/terminal-enduser-email-consent-denied.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
