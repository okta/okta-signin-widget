...
describe('usePolling', () => {
  ...
  describe('idxTransaction includes polling step - returns transaction and setup polling timer', () => {
    ...
    it('updates refresh interval when polling step changes and stops after terminal step (challenge-poll as last)', async () => {
      const initialIdxTransaction = {
        // @ts-ignore remove after adding refresh to nextStep in auth-js
        nextStep: {
          name: 'challenge-poll',
      ...
      // Fourth proceed -> challenge-poll (4000)
      jest.advanceTimersByTime(4000);
      await waitFor(() => {
        expect(mockProceedFn).toHaveBeenCalledTimes(4);
        expect(result.current).toBeUndefined();
      });
