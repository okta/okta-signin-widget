...
export const useOnSubmit = (): ((
  options: OnSubmitHandlerOptions
) => Promise<void>) => {
  const {
    authClient,
    data,
  ...
  return useCallback(
    async (options: OnSubmitHandlerOptions) => {
      setLoading(true);
      ...
      const getFormPasswordAuthenticatorId = (
        transaction: IdxTransaction
      ): string | undefined =>
        transaction?.neededToProceed
          ?.find(
            remediation =>
              remediation.name === IDX_STEP.SELECT_AUTHENTICATOR_AUTHENTICATE
          )
          ?.value?.find(val => val.name === "authenticator")
          ?.options?.find(option => option.label === "Password")
          // @ts-expect-error auth-js type errors
          ?.value?.form?.value?.find(formVal => formVal.name === "id")?.value;
      ...
      const getIsClientTransaction = (newTx: IdxTransaction) => {
        ...
        return (
          (!newTx.requestDidSucceed &&
            // do not preserve field data on token change errors
            !containsMessageKey(
              ON_PREM_TOKEN_CHANGE_ERROR_KEY,
              newTx.messages
            )) ||
          (areTransactionsEqual(currTransaction, newTx) &&
            transactionHasWarning)
        );
      };

      const onSuccess = (
        newTx: IdxTransaction,
      ...
      const handleError = (
        ...
        if (authApiError?.xhr?.responseText) {
          if (typeof authApiError.xhr.responseText === "string") {
            try {
              transactionErrorResponse = JSON.parse(
                authApiError.xhr.responseText
              );
            } catch (e) {
      ...
      if ("identifier" in payload) {
        payload.identifier = transformIdentifier(
          widgetProps,
          step,
      ...
      const keysWithoutNesting =
        step === IDX_STEP.CONSENT_GRANULAR ? ["optedScopes"] : [];
      ...
      if (step === IDX_STEP.IDENTIFY && features?.deviceFingerprinting) {
        ...
        if (baseUrl) {
          ...
          const fingerprint = await generateDeviceFingerprint(baseUrl).catch(
            () => undefined
          );
          if (fingerprint) {
      ...
      try {
        ...
        // OKTA-651781
        if (
          isConfigRecoverFlow(widgetProps.flow) &&
          ...
          newTransaction = await authClient.idx.proceed({
            ...
            authenticator: {
              id: getFormPasswordAuthenticatorId(newTransaction)
            },
        ...
        if (step === IDX_STEP.ENROLL_PROFILE && !isClientTransaction) {
          const postRegistrationSubmitPromise = new Promise(resolve => {
            postRegistrationSubmit(
              ...
              _response => {
                onSuccess(newTransaction, resolve);
              },
              error => {
                triggerRegistrationErrorMessages(
                  error,
