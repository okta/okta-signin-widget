...
describe('WebAuthN Transformer Tests', () => {
  ...
  describe('WebAuthN Enroll Tests', () => {
    ...
    it('should render title, description and button elements when WebAuthN API is available', () => {
      ...
      expect(updatedFormBag.uischema.elements[2]?.options?.onClick).not.toBeUndefined();
      expect(updatedFormBag.uischema.elements[2]?.options?.label).toBe('oie.enroll.webauthn.save');
      expect(updatedFormBag.uischema.elements[2]?.options?.submitOnLoad).toBe(true);
      expect(updatedFormBag.uischema.elements[2]?.options?.showLoadingIndicator).toBe(true);
      expect(updatedFormBag.uischema.elements[2]?.options?.action).not.toBeUndefined();
    });
  });
  ...
  describe('WebAuthN Challenge Tests', () => {
    ...
    it('should render title, description and button elements when WebAuthN API is available', () => {
      ...
      expect(updatedFormBag.uischema.elements[2]?.type).toBe('Button');
      expect(updatedFormBag.uischema.elements[2]?.options?.onClick).not.toBeUndefined();
      expect(updatedFormBag.uischema.elements[2]?.options?.label).toBe('mfa.challenge.verify');
      expect(updatedFormBag.uischema.elements[2]?.options?.submitOnLoad).toBe(true);
      expect(updatedFormBag.uischema.elements[2]?.options?.showLoadingIndicator).toBe(true);
      expect(updatedFormBag.uischema.elements[2]?.options?.action).not.toBeUndefined();
    });
