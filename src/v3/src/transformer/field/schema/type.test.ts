...
import { Result, transformer } from 'src/transformer/field/schema/type';
...
describe('"type" field transformer', () => {
  it('should add type to schema.properties.{fieldname} if type is present in ION formfield', () => {
    const formfield: IonFormField = { name: 'foo', type: 'boolean' };
    const result: Result = {
      foo: {
        type: 'boolean',
      },
  ...
  it('should add set type to string as default if "type" field is not present in ION formfield', () => {
    const formfield: IonFormField = { name: 'foo' };
    const result = {
      foo: {
        type: 'string',
      },
