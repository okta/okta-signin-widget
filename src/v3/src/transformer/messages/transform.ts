...
export const OV_OVERRIDE_MESSAGE_KEY: Record<string, string> = {
  ...
  OV_QR_ENROLL_ENABLE_BIOMETRICS_OR_PIN_KEY: 'oie.authenticator.app.method.push.enroll.enable.biometrics_or_pin',
};
...
const MESSAGE_KEYS_WITH_TITLE = [
  ...
  OV_OVERRIDE_MESSAGE_KEY.OV_QR_ENROLL_ENABLE_BIOMETRICS_OR_PIN_KEY,
  OV_NMC_FORCE_UPGRADE_SERVER_KEY,
  NFC_SIGN_IN_TO_ENROLL_SERVER_KEY,
  NFC_UNAVAILABLE_SERVER_KEY,
  FASTPASS_BLOCKED_SHARED_DEVICE_SERVER_KEY,
...
const transformMessagesWithTitle: TransformStepFnWithOptions = ({
  ...
  if (containsOneOfMessageKeys(fipsComplianceKeys, messagesWithTitle)) {
    ...
    widgetMessage.title = loc('oie.authenticator.app.method.push.enroll.enable.biometrics_or_pin.title', 'login');
