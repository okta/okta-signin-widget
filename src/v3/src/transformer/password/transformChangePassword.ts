...
import {
  ...
  const getContentTitleAndParams = (brandName?: string): TitleElement['options'] => {
    if (brandName) {
      return { content: loc('password.change.title.specific', 'login', [brandName]) };
    }
    return { content: loc('password.change.title.generic', 'login') };
  };
  
  export const transformChangePassword: IdxStepTransformer = ({
    transaction,
    formBag,
    widgetProps,
  }) => {
    ...
    const submitBtnElement: ButtonElement = {
      type: 'Button',
      label: loc('password.change.submit', 'login'),
      options: {
    ...
    // Move current password up to the top
    let currentPasswordField = uischema.elements.splice(6, 1)[0];
    uischema.elements.splice(3, 0, currentPasswordField);
