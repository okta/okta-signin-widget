...
describe('EmailChallengeConsentTransformer Tests', () => {
  ...
  it('should create email consent ui elements with valid response', () => {
    const updatedFormBag = transformEmailChallengeConsent(transaction, formBag);
    ...
    expect(formBag.uischema.elements[3].options?.idxMethod).toBe('proceed');
    expect(formBag.uischema.elements[3].options?.idxMethodParams?.consent).toBeFalsy();

    expect(formBag.uischema.elements[4].options?.format).toBe('button');
    expect(formBag.uischema.elements[4].options?.idxMethod).toBe('proceed');
    expect(formBag.uischema.elements[4].options?.idxMethodParams?.consent).toBeTruthy();
  });
});
