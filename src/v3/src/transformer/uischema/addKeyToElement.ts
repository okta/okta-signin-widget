...
import { TransformStepFnWithOptions } from 'src/types';
import { generateRandomString, getAuthenticatorKey, getCurrentAuthenticator } from 'src/util';
    ...
    callback: (element) => {
      ...
      if (element.type === 'Reminder') {
        ...
        let elementKey = element.key
          ...
          : `${name}_${element.type}`;
        elementKey += [authenticatorKey, authId].filter(Boolean).map((str) => `_${str}`).join('');
        // eslint-disable-next-line no-param-reassign
        element.key = elementKeySet.has(elementKey) ? `${elementKey}_${generateRandomString()}` : elementKey;
        elementKeySet.add(elementKey);
      }
    },
  });
  return formbag;
