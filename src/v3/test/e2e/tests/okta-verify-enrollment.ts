...
fixture('Okta Verify Authenticator Enrollment')
...
test('User can enroll using Okta Verify QRCode Scan to enroll', async (t) => {
  ...
  assertRedirectionToAuthenticatorList(t);
});
...
test('User can enroll using Okta Verify SMS text link', async (t) => {
  ...
  const countrySelect = Selector('div').withAttribute('role', 'listbox').nth(0);
  const countryOption = countrySelect.find("div[role='option']");
  await t
    .click(countrySelect)
    .click(countryOption.withText('United States'))
    .expect(Selector('div').withExactText('United States').exists).ok();
