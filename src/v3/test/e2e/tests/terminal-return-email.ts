...
test('Successful email link verification view, prompt to return to original tab', async (t) => {
  ...
  await t
    .expect(Selector('h2').withText('Success! Return to the original tab or window').exists)
    .ok();

  const paragraph = Selector('p');
  await t
    .expect(
      paragraph.withText('To continue, please return to the original browser tab or window you used to verify.')
        .exists,
    )
    .ok();
  await t
    .expect(paragraph.withText('Close this window anytime.').exists)
    .ok();
