...
scenario('authenticator-expired-password-no-complexity', (rest) => ([
  // bootstrap
  ...
  // expired password with no complexity
  rest.post('*/idp/idx/identify', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/identify/authenticator-expired-password-no-complexity.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
