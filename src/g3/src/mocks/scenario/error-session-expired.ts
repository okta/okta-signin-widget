...
scenario('error-session-expired', (rest) => ([
  // bootstrap
  ...
  // return session expired after submitting profile credentials
  rest.post('*/idp/idx/identify', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/identify/error-session-expired.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
