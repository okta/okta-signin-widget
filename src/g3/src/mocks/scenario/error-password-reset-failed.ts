...
scenario('error-password-reset-failed', (rest) => ([
  // bootstrap
  ...
  // returns error after entering code
  rest.post('*/idp/idx/challenge/answer', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/challenge/answer/error-password-reset-failed.json');
    return res(
      ctx.status(200),
      ctx.json(body),
    );
