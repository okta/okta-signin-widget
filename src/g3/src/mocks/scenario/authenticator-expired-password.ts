...
scenario('authenticator-expired-password', (rest) => ([
  // bootstrap
  ...
  // expired password with complexity
  rest.post('*/idp/idx/identify', async (req, res, ctx) => {
    const { default: body } = await import('../response/idp/idx/identify/authenticator-expired-password.json');
    return res.once(
      ctx.status(200),
      ctx.json(body),
    );
