...
import { omit, map, sortBy, each } from 'lodash';
import { loc } from './loc';
...
fn.getCountries = function() {
  const countries = omit(bundles.country, 'HM', 'BV', 'TF');
  ...
  let collection = map(countries, function(name, code) {
  ...
  collection = sortBy(collection, 'name');
  const sorted = {};

  each(collection, function(country) {
